**Задача:**
выделить замкнутые структурные ловушки по структурной карте глубин кровли геологического горизонта, структурная ловушка - это закрашенная часть карты по последней замкнутой изолинии, выше которой на карте существует замкнутая возвышенность.


**Данные:**
1. ВАРИАНТ: изображения структурных карт, карт разломов и т.д.
    * X:
        1. rgb: структурная карта глубины кровли геологического горизонта, раскрашенная в цветовую палитру от синего (самые низкие части карты) до красного (самые высокие части карты) без изолиний + коричневые толстые разломы.
            - Показать модели рельеф.
            - Мин и макс цветовой шкалы локальные для каждой карты. 
            - Имя в файле: x_structuralNOisoline
            - Формат png
        2. fault_mask: карта разломов - черный цвет это разломы, белый цвет это все остальное. 
            ...
            - Имя в файле: x_faults
            - Формат png
        3. depth_norm: структурная карта глубины кровли геологического горизонта без изолиний и без разломов в черно-белой палитре (черный цвет (0) - наивысшие точки на карте, белый цвет (255) - самые низкие точки на карте) 
            ...
            - Имя в файле: x_structuralBlackWhite
            - Формат png
    * Y:
...
2. ВАРИАНТ: 2D гриды структурных карт в формате cps без изолиний
    * X:
        1. rgb: цветное изображение 2D грида структурной карты, раскрашенная в цветовую палитру от синего (самые низкие части карты) до красного
            - Имя в файле: x_structuralNOisoline
            - Формат cps 
        2. fault_mask: изображение 2D грида карты разломов - черный цвет это разломы, белый цвет это все остальное.
            - Имя в файле: x_faults
            - Формат cps
        3. depth_norm: черно-белое изображение 2D грида структурной карты
            - Имя в файле: x_structuralBlackWhite
            - Формат cps
    * Y:
        1. Черно-белое изображение 2D грида карты выделенных замкнутых структурных ловушек, где черный цвет - сами ловушки, белый цвет  - все остальное. 
            - Имя в файле: y_traps
            - Формат cps
* Вспомогательные маски для управления обучения:
    1. depth_mask: маска для depth_norm, где черный цвет - это границы карта без разломов, а белый цвет - это пустоты под разломами. 
        - В черно-белой структурной карте без изолиний и разломов depth_norm, потому что под разломами фактически нет карты, т.к. разломы в основном сбросы (normal faults), т.е. они "раздвигают" кровлю горизонта, под ними будут белые пятна - зияние.
...
**Режимы данных:**
* Без разломов: rgb карта без изолиний и БЕЗ разломов + depth map
* С разломами: rgb карта без изолиний и С разломов + fault map + depth map
...
* Для ResNet34 меняем in_channels = 5 (rgb + fault_mask + depth_norm) или 4 (rgb + depth_norm).
...
1. RGB без изолиний + depth_norm + map_mask
...
* clip_grad_norm = 5.0
* Попробовать DeepLabV3+, HRNet или SegFormer (надо со входом поработать, там тоже первый слой ожидает 3 канала, плюс сделать ширину и высоту изображений кратным 32: например, 1280х512) архитектуры.
* LR до 5е-5
