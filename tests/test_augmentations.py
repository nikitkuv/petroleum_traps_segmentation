...
class TestCpsTilesDatasetInitialization:
    ...
    def sample_cps_tiles_files(self, tmp_path):
        ...
        # Create minimal complete dataset with CPS-style naming (5 required files)
        filenames = [
            "001_x_structuralNOisoline_Ach3-2-1_toptop1.png",
            "001_x_structuralBlackWhite_Ach3-2-1_toptop1.png",
    ...
    def test_cps_tiles_dataset_without_faults_excludes_fault_channel(self, sample_cps_tiles_files):
        ...
        # Input should have 6 channels (RGB=3 + depth=1 + isolines=1 + closed_isolines=1)
...
class TestCpsTilesDatasetGetItem:
    ...
    def complete_cps_sample(self, tmp_path):
        ...
        # Create isolines image
        isolines_img = np.zeros((100, 100), dtype=np.uint8)
        isolines_img[20:80, 20:80] = 255  # White square
        cv2.imwrite(str(data_dir / "001_x_isolines_Ach3-2-1_toptop1.png"), isolines_img)

        # Create closed isolines image
        closed_isolines_img = np.zeros((100, 100), dtype=np.uint8)
        closed_isolines_img[30:70, 30:70] = 255  # Smaller white square
        cv2.imwrite(str(data_dir / "001_x_closedIsolines_Ach3-2-1_toptop1.png"), closed_isolines_img)
        ...
        file_paths = [
            ...
            str(data_dir / "001_x_isolines_Ach3-2-1_toptop1.png"),
            str(data_dir / "001_x_closedIsolines_Ach3-2-1_toptop1.png"),
    ...
    def test_cps_tiles_getitem_input_channels_with_faults(self, complete_cps_sample):
        ...
        # Should have 7 channels: RGB(3) + depth(1) + isolines(1) + closed_isolines(1) + faults(1)
        assert sample['x'].shape[0] == 7
    ...
    def test_cps_tiles_getitem_input_channels_without_faults(self, complete_cps_sample):
        ...
        # Should have 6 channels: RGB(3) + depth(1) + isolines(1) + closed_isolines(1)
        assert sample['x'].shape[0] == 6
...
class TestCpsTilesDataLoaderCreation:
    ...
    def sample_cps_data_for_loader(self, tmp_path):
        ...
        for card_num, horizon in sample_configs:
            ...
            # Isolines
            isolines_img = np.zeros((100, 100), dtype=np.uint8)
            isolines_img[20:80, 20:80] = 150
            cv2.imwrite(str(data_dir / f"{card_num:03d}_x_isolines_{horizon}.png"), isolines_img)

            # Closed isolines
            closed_isolines_img = np.zeros((100, 100), dtype=np.uint8)
            closed_isolines_img[30:70, 30:70] = 180
            cv2.imwrite(str(data_dir / f"{card_num:03d}_x_closedIsolines_{horizon}.png"), closed_isolines_img)
    ...
    def test_cps_tiles_dataloader_batch_shapes(self, sample_cps_data_for_loader):
        ...
        assert batch['x'].shape[1] == 6  # Channels (RGB=3 + depth=1 + isolines=1 + closed_isolines=1)
...
def test_cps_tiles_smoke_test(tmp_path):
    ...
    # Create minimal CPS tile sample (5 required files)
    for filename in [
        "001_x_structuralNOisoline_Ach3-2-1_toptop1.png",
        "001_x_structuralBlackWhite_Ach3-2-1_toptop1.png",
        "001_x_isolines_Ach3-2-1_toptop1.png",
        ...
        if 'BlackWhite' in filename or 'isolines' in filename or 'traps' in filename:
            img = img[:, :, 0]
        cv2.imwrite(str(data_dir / filename), img)
