...
class GeologyTrapsDataset(Dataset):
    def __init__(
        ...
        print(f"Mode: use_faults={self.use_faults}, use_rgb={self.use_rgb}")
        ...
        n_required = 5 if self.use_rgb else 2
        n_faults_msg = " + faults" if self.use_faults else ""
        rgb_msg = "rgb, " if self.use_rgb else ""
        print(f"Required files per sample: {n_required}{n_faults_msg} ({rgb_msg}depth_norm, isolines[n_faults_msg], traps)")
    ...
    def _parse_files(self, file_list: List[str]) -> List[Dict[str, str]]:
        ...
        for key, paths in samples.items():
            required_keys = ['depth_norm', 'isolines', 'traps']
            if self.use_rgb:
                required_keys.insert(0, 'rgb')
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        augmented = self.transforms(
            image=rgb_padded if self.use_rgb else rgb_norm,
            depth=depth_padded,
        ...
        # Извлекаем тензоры
        x_rgb = augmented['image'] if self.use_rgb else None
        ...
        input_tensors = []
        if self.use_rgb:
            input_tensors.append(x_rgb)
        input_tensors.extend([x_depth, x_isolines])
        if self.use_faults:
            input_tensors.append(x_faults)
        input_tensors.append(mask_map)
        x_in = torch.cat(input_tensors, dim=0)
